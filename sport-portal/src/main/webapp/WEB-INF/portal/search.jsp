			...
			<c:if test="${fn:length(map) != 0 }">
                ...
                <div class="sl-b-selected J_brandSelected">
                    <c:forEach items="${map }" var="m">
                        <a title="依琦莲（yiqilian）"  href="javascript:delQuery('${m.value}');" class="crumb-select-item">
...
<script type="text/javascript">
    ...
    function fqPrice(priceSelect){
        window.location.href = "/Search?keyword="+keyword+"&brandId="+brandId+"&price="+priceSelect;
    }

    function delQuery(condition){
        if(condition == brandName){
            window.location.href = "/Search?keyword="+keyword+"&price="+price;
        }
        if(condition == price || condition == price+"以上"){
